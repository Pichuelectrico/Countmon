...
class PointWidget(QtWidgets.QWidget, WIDGET):
    ...
    def display_classes(self):
        ...
        for row, class_name in enumerate(self.canvas.classes):
            ...
            self.tableWidgetClasses.setItem(
                row, COL_NAME, QtWidgets.QTableWidgetItem(class_name)
            )
    ...
    def export(self):
        if self.radioButtonCounts.isChecked():
            ...
            fn = QtWidgets.QFileDialog.getSaveFileName(
                self,
                self.tr("Export Image With Points"),
    ...
    def select_model_item(self, model_index):
        ...
        if item.isSelectable():
            if item.column() != 0:
                item = self.model.itemFromIndex(self.model.index(item.row(), 0))
            self.canvas.load_image(os.path.join(self.canvas.directory, item.text()))

    def selection_changed(self, selected, deselected):
        if len(selected.indexes()) > 0:
            row = selected.indexes()[0].row()
            self._last_class_row = row
    ...
    def deselect_class(self):
        """Clear the active class selection (ESC)."""
        self.tableWidgetClasses.selectionModel().clear()

    def restore_last_class(self):
        """Re-select the last used class when entering Counter mode.

        Falls back to the first class if no class was previously used or the
        remembered row is out of range.
        """
        if self.tableWidgetClasses.rowCount() == 0:
            return
        row = self._last_class_row
        if row is None or row >= self.tableWidgetClasses.rowCount():
            row = 0
        self.tableWidgetClasses.selectRow(row)
    ...
    def update_point_count(self, image_name, class_name, class_count):
        ...
        if len(items) == 0:
            ...
            items[0].child(self.canvas.classes.index(class_name), 1).setText(
                str(class_count)
            )
    ...
    def update_ui_settings(self):
        ui = self.canvas.ui
        color = QtGui.QColor(
            ui["point"]["color"][0], ui["point"]["color"][1], ui["point"]["color"][2]
        )
        self.set_active_point_color(color)
        self.spinBoxPointRadius.setValue(ui["point"]["radius"])
        color = QtGui.QColor(
            ui["grid"]["color"][0], ui["grid"]["color"][1], ui["grid"]["color"][2]
        )
        self.set_grid_color(color)
